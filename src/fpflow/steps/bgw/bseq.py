...
# Get the directories. 
bseq_dirs = [inode for inode in glob.glob('./q_*') if os.path.isdir(inode)]
bseq_dirs.sort()
start: int = 0
...
for bseq_dir in bseq_dirs[start:stop]:
    kernel_script = os.path.join(bseq_dir, 'kernel', 'job_kernel.sh')
    absorption_script = os.path.join(bseq_dir, 'absorption', 'job_absorption.sh')
    # plotxct_script = os.path.join(bseq_dir, 'plotxct', 'job_plotxct.sh')

    total_time = subprocess_run(kernel_script, total_time=total_time)
    total_time = subprocess_run(absorption_script, total_time=total_time)
    # total_time = subprocess_run(plotxct_script, total_time=total_time)
...
'''
    ...
    def _create_in_subdir(self, Qpt: list, Qpt_idx: int):
        ...
        # Change parameters. 
        glom.assign(inputdict_local, 'bse.link_dir_prefix', '../../../')
        ...
        glom.assign(inputdict_local, 'generator.steps', ['kernel_bgw', 'absorption_bgw'])      # Removing plotxct for now. 
        ...
        # glom.assign(inputdict_local, 'manager.steps', ['kernel_bgw', 'absorption_bgw', 'plotxct_bgw']) # Removing plotxct for now.
        glom.assign(inputdict_local, 'manager.steps', ['kernel_bgw', 'absorption_bgw'])
