...
#region: Functions.
def get_nqpt():
    fname = './_ph0/struct.phsave/control_ph.xml'
...
def get_first_nonzero_dvscf_file(qpt_idx):
   ...
   if qpt_idx==1:
      return './_ph0/struct.dvscf1'
   else:
      glob_pat = f'./**/struct.q_{qpt_idx}/struct.dvscf1'
...
def move_data_to_save():
   ...
   os.system('cp -r ./_ph0/struct.phsave save/')
   ...
   for qpt_idx in range(1, get_nqpt()+1):
      ...
      # Delete some files. 
      # os.system(f'rm ./**/struct.q_{qpt_idx}/*wfc*' )
