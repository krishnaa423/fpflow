...
class EpwXctphPlot(PlotBase):
    ...
    def get_data(self):
        ...
        exband = np.loadtxt('./xctph_epw/G_full_epmatq/exband.fmt')
        ...
        change = change + self.exbands[:, 0]

        change.reshape(-1, 1)

        ste_val: np.ndarray = (7.0 -0.431)* np.ones(shape=(21,))
        ste_val.reshape(-1, 1)
        ...
        # bmat_path = './xctph_epw/bmat.band.plrn'
        ...
        df = pd.DataFrame(
            ...
            np.hstack([self.axis, self.exbands, change, ste_val]),
            # columns=["x"] + self.exband_colnames + self.exband_weight_colnames
            columns=["x"] + self.exband_colnames + ['change', 'ste_val']
        )
        ...
        df_ph = pd.DataFrame(
            ...
            columns=["x"] + self.phband_colnames
        )
    ...
    def set_figures(self):
        for ib in range(self.num_exbands):
            ...
            # Add polaron band. 
            if ib == 0:
                append_ste_fig_df: pd.DataFrame = pd.DataFrame([
                    {
                        'fig_name': 'zd_ste_band',
                        'figure': None, 'subplot_nrow': 1, 'subplot_ncol': 1, 'subplot_idx': 1,
                        'plot_type': PlotType.LINE, 'axis': None,
                        'xlabel': None, 'xlim': (self.xaxis[0], self.xaxis[-1]), 'xticks': self.xticks, 'xtick_labels': self.xtick_labels,
                        'ylabel': 'Energy (eV)', 'ylim': None, 'yticks': None, 'ytick_labels': None,
                        'zlabel': None, 'zlim': None, 'zticks': None, 'ztick_labels': None,
                        'z_inc': None, 'z_azim': None,
                        'title': f'{self.struct_name} STE Band',
                        'dset_name': 'dset_exbands',
                        'dset_axis_cols': 'x',        
                        'dset_data_cols': ['change'],
                        'color': 'blue', 
                ...
                self.figs_df = pd.concat([self.figs_df, append_ste_fig_df], ignore_index=True)
